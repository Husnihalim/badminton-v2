-- Migration: Fix get_player_dashboard RPC for PG17 compatibility
-- Fix 1: m.created_at → m.joined_at (memberships table has joined_at, not created_at)
-- Fix 2: Use subquery wrapping for jsonb_agg(jsonb_build_object(...)) to avoid PG17 GROUP BY strictness
-- Created: 2026-06-12
