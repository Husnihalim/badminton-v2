...
import FindYourScene from '../components/home/FindYourScene'
import FacesOfCommunity from '../components/home/FacesOfCommunity'
import WinsAndHeartbreaks from '../components/home/WinsAndHeartbreaks'
import MarketplaceRail from '../components/home/MarketplaceRail'
import { buildTickerItems } from '../components/home/formatMatch'

const allMockStories = [...mockStories['mock-lep-bc'], ...mockStories['mock-smashers-pj']]
  .sort((a, b) => (b.priority ?? 0) - (a.priority ?? 0))
const leadStory = allMockStories[0] ?? null
const secondaryStories = allMockStories.slice(1, 4)
const tickerItems = buildTickerItems(mockMatches)
...
export default function LandingPage() {
  ...
  return (
    <Page>
      {/* 1. LIVE SCORE TICKER */}
      <LiveScoreTicker items={tickerItems} />

      {/* 2. LEAD STORY HERO */}
      <LeadStoryHero
        leadStory={leadStory}
        secondaryStories={secondaryStories}
        heroImage={heroBadminton}
      />

      {/* 3. FIND YOUR SCENE — location-aware club / marketplace search */}
      <FindYourScene />

      {/* 4. FACES OF THE COMMUNITY — opt-in player rail */}
      <FacesOfCommunity />

      {/* 5. WINS & HEARTBREAKS — emotional editorial split */}
      <WinsAndHeartbreaks stories={allMockStories} />

      {/* 6. MARKETPLACE PREVIEW — 3 deal-of-the-week near you */}
      <MarketplaceRail />

      {/* 7. Soft pitch band — keeps a clear way in for new visitors */}
      <section className="landing-hero relative overflow-hidden rounded-2xl border border-[var(--arena-border)] bg-[var(--arena-surface)] p-6 md:p-10 shadow-[var(--arena-glow)]">
        ...
        <div className="landing-hero-copy z-10 flex flex-col justify-center items-start text-left">
          ...
          <h2 className="landing-hero-title text-3xl md:text-5xl font-black text-[var(--arena-text)] uppercase tracking-tight leading-none mb-4">
            The Grassroots <br/>
      ...
      {/* 7. PLAYGROUND PROMO — soft link to /playground */}
      <section className="pt-10">
        <Link
          to="/playground"
          className="block rounded-2xl border border-[var(--arena-border)] bg-[var(--arena-surface)] p-5 md:p-6 hover:border-[var(--arena-accent)] transition-colors"
        >
          <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--arena-accent-soft)] px-3 py-1 text-xs font-bold text-[var(--arena-accent)] border border-[var(--arena-accent)]/20 uppercase tracking-wider">
              <ClipboardPenLine size={12} />
              Playground
            </span>
            <div className="flex-1">
              <h3 className="text-lg md:text-xl font-black text-[var(--arena-text)] uppercase tracking-tight">
                Curious how it feels?
              </h3>
              <p className="text-sm text-[var(--arena-text-muted)] mt-1 max-w-xl">
                Try the score recorder and design-a-club widgets on the playground page — no signup needed.
              </p>
            </div>
            <span className="inline-flex items-center gap-1 text-sm font-bold text-[var(--arena-accent)] uppercase tracking-wide">
              Open playground
