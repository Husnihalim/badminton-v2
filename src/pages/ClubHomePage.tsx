            ...
            {matches.length ? (
              <div className="space-y-3">
                {matches.slice(0, 5).map((match) => {
                  ...
                  return (
                    <div key={match.id} className={`relative rounded-xl border bg-[#0b1322] border-slate-800 overflow-hidden shadow-lg transition duration-200 hover:border-slate-700 ${
                      ...
                      <div className="flex items-center justify-between px-4 py-2.5 bg-[#070b13] border-b border-slate-800/60">
                        <div>
                          ...
                          <span className="text-[10px] text-slate-500 font-medium">
                            {match.sport} • {match.match_type} • {new Date(match.match_date).toLocaleDateString()}
                          </span>
                        </div>
                        
                        {/* Actions */}
                        <div className="flex items-center gap-1.5">
                          <Button type="button" variant="ghost" size="icon" onClick={() => setShareMatch(match)} title="Share Scorecard" className="h-7 w-7 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg">
                            <Share2 size={13} aria-hidden="true" />
                          </Button>
                          {isAdmin ? (
                            <>
                              <Button type="button" variant="ghost" size="icon" onClick={() => handleEditMatch(match)} title="Edit score" className="h-7 w-7 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg">
                                <Pencil size={13} aria-hidden="true" />
                              </Button>
                              <Button type="button" variant="ghost" size="icon" onClick={() => handleDeleteMatch(match.id)} title="Delete score" className="h-7 w-7 text-red-400 hover:text-red-300 hover:bg-red-950/45 rounded-lg">
                      ...
                      <div className="p-4 flex items-center justify-between gap-4">
                        ...
                        <div className="space-y-3.5 flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2">
                            {winner === 1 && <span className="text-amber-400 font-bold shrink-0 animate-pulse" title="Winner">👑</span>}
                            <div className={`truncate text-sm md:text-base font-extrabold uppercase tracking-wide flex flex-wrap items-center gap-1.5 ${
                                ...
                                return (
                                  <span key={pIdx} className="flex items-center">
                                    {matchingMember?.user_id ? (
                                      <Link to={`/member/${matchingMember.user_id}`} className="hover:underline">
                                        {pName}
                                      </Link>
                                    ) : (
                                      <span>{pName}</span>
                          ...
                          {/* Row Team 2 */}
                          <div className="flex items-center gap-2">
                            {winner === 2 && <span className="text-amber-400 font-bold shrink-0 animate-pulse" title="Winner">👑</span>}
                            <div className={`truncate text-sm md:text-base font-extrabold uppercase tracking-wide flex flex-wrap items-center gap-1.5 ${
                          ...
                          {scoreSets.map((set, setIdx) => {
                            ...
                            return (
                              <div key={setIdx} className="flex flex-col gap-1 text-center">
                                <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest block mb-0.5">SET {set.set_number}</span>
                                
                                {/* Team 1 Box */}
                                <div className={`w-10 h-10 rounded flex items-center justify-center font-mono font-black text-lg border transition ${
                                  t1Won 
